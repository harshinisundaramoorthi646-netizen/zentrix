...
export const Sidebar: React.FC<SidebarProps> = ({ collapsed, setCollapsed }) => {
  ...
  return (
    <aside
      className={`fixed top-0 left-0 bottom-0 z-30 bg-[#0D1118]/95 backdrop-blur-2xl border-r border-white/10 transition-all duration-300 flex flex-col justify-between ${
        collapsed ? 'w-20' : 'w-64'
      }`}
    >
      {/* Top Branding */}
      <div className="p-4 border-b border-white/10 flex items-center justify-between">
        {!collapsed && <ZentrixLogo size="sm" />}
        {collapsed && <ZentrixLogo size="sm" showSubtitle={false} className="justify-center mx-auto" />}
        ...
        {filteredNavItems.map((item) => {
          ...
          return (
            <button
              ...
              className={`w-full flex items-center gap-3 px-3.5 py-2.5 rounded-2xl font-medium text-sm transition-all duration-300 group ${
                isActive
